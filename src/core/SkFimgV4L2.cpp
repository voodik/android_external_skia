...
int ChangeColorForV4L2(int colorFormat) {
    if (colorFormat == SkBitmap::kRGB_565_Config)
        return V4L2_PIX_FMT_RGB565;
    if (colorFormat == SkBitmap::kARGB_8888_Config)
                    ...
                    size_t rowbyte, uint32_t color)
{
    ...
    result = exynos_bl_set_dst_format(fimg_handle_t, rowbyte/32, y+height, x, y, width, height, ChangeColorForV4L2(SkBitmap::kARGB_8888_Config));
    if (result < 0) {
        ALOGE("<SkFimgV4L2> exynos_bl_set_dst_format error : %d", result);
        exynos_bl_destroy(fimg_handle_t);
        return false;
                    ...
                    size_t rowbyte, uint32_t color)
{
    ...
    result = exynos_bl_set_dst_format(fimg_handle_t, rowbyte/16, y+height, x, y, width, height, ChangeColorForV4L2(SkBitmap::kRGB_565_Config));
    if (result < 0) {
        ALOGE("<SkFimgV4L2> exynos_bl_set_dst_format error : %d", result);
        exynos_bl_destroy(fimg_handle_t);
        return false;
